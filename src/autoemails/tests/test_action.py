...
import src.autoemails.bulk_email
from src.autoemails.actions import BaseAction
from src.autoemails.bulk_email import send_bulk_email
...
class BulkEmailTest(FakeRedisTestCaseMixin, TestCase):
    ...
    def setUp(self):
        ...
        self._saved_scheduler = src.autoemails.bulk_email.scheduler
        # overwrite them
        src.autoemails.bulk_email.scheduler = self.scheduler

    def tearDown(self):
        super().tearDown()
        src.autoemails.bulk_email.scheduler = self._saved_scheduler
    ...
    def test_send_bulk_email(self) -> None:
        ...
        expected_sent_emails = [
            emails[i : i + settings.BULK_EMAIL_LIMIT]
            for i in range(0, len(emails), settings.BULK_EMAIL_LIMIT)  # noqa
        ]
        for job, expected_emails in zip(rq_jobs, expected_sent_emails, strict=False):
