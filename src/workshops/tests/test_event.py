...
from src.workshops.forms import EventCreateForm, EventForm, EventsMergeForm
...
class TestEvent(TestBase):
    ...
    def test_repository_website_url(self) -> None:
        ...
        for index, link in enumerate(links):
            event = Event.objects.create(slug=f"e{index}", host=test_host, url=link)
...
class TestEventViews(TestBase):
    ...
    def test_add_minimal_event(self) -> None:
        ...
        self.assertEqual(
            event.host,
            host,
            f"New event has wrong host: {event.host} != {host}",
        )
        tags = list(event.tags.all())
        self.assertEqual(
            tags,
            [self.test_tag],
            f"New event has wrong tags: {tags} != {[self.test_tag]}",
        )
    ...
    def test_slug_illegal_characters(self) -> None:
        ...
        for slug_suffix in ["a/b", "a b", "a!b", "a.b", "a\\b", "a?b", "aób"]:
            with self.subTest(slug_suffix=slug_suffix):
                data["slug"] = f"2016-06-30-{slug_suffix}"
                f = EventForm(data)
