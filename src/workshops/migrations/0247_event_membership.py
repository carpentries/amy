...
def select_membership_for_events(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    ...
    * https://github.com/carpentries/amy/issues/1941
    ...
    for event in events:
        ...
        if not start:
            print(f"Event {event} cannot be matched to membership: start date is unknown.")
            continue

        # If sponsor is unknown, inform the user.
        if not event.sponsor:
            print(f"Event {event} cannot be matched to membership: sponsor is unknown.")
            continue

        # Find memberships for selected sponsor where event start lies.
        memberships = event.sponsor.memberships.filter(agreement_start__lte=start, agreement_end__gte=start)
