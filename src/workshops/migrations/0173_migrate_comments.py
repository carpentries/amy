...
def migrate_notes(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    ...
    default_submit_date = datetime.now(UTC)

    def comment_factory(content_type: Any, object_pk: int, comment: str) -> Any:
        return Comment(
    ...
    for organization in organizations:
        comments.append(comment_factory(organization_ct, organization.pk, organization.notes))
    for membership in memberships:
        comments.append(comment_factory(membership_ct, membership.pk, membership.notes))
    for request in trrequests:
        comments.append(comment_factory(trrequest_ct, request.pk, request.notes))
    for request in wrequests:
        comments.append(comment_factory(wrequest_ct, request.pk, request.admin_comment))
