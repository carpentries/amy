...
def rename_discussion_to_welcome_session(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
...
def rename_welcome_session_to_discussion(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    TrainingRequirement = apps.get_model("workshops", "TrainingRequirement")
...
def migrate_outdated_requirements(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    TrainingRequirement = apps.get_model("workshops", "TrainingRequirement")
    ...
    progresses.filter(requirement__name__endswith="Homework").update(requirement=contribution)
