...
def sample[T](population: Sequence[T], k: int | None = None) -> list[T]:
...
class Command(BaseCommand):
    ...
    def fake_roles(self) -> None:
        ...
        self.stdout.write(f"Generating {len(roles)} fake roles...")
    ...
    def fake_tags(self) -> None:
        ...
        self.stdout.write(f"Generating {len(tags)} fake tags...")
    ...
    def fake_instructors(self, count: int = 30) -> None:
        self.stdout.write(f"Generating {count} fake instructors...")
    ...
    def fake_trainers(self, count: int = 10) -> None:
        self.stdout.write(f"Generating {count} fake trainers...")
        for _ in range(count):
            try:
                with transaction.atomic():
    ...
    def fake_admins(self, count: int = 10) -> None:
        self.stdout.write(f"Generating {count} fake admins...")
        for _ in range(count):
            try:
                with transaction.atomic():
    ...
    def fake_trainees(self, count: int = 30) -> None:
        self.stdout.write(f"Generating {count} fake trainees (and their training progresses and training requests)...")
        for _ in range(count):
            try:
                with transaction.atomic():
    ...
    def fake_training_request(self, person_or_None: Person | None) -> None:
        ...
        if registration_code and randbool(0.5):
            eventbrite_url = f"https://eventbrite.com/fake-{self.faker.random_number(digits=12, fix_len=True)}"
    ...
    def fake_person(self, *, is_instructor: bool, is_trainer: bool = False) -> Person:
        ...
        username = create_username(personal_name, family_name) if randbool(0.6) else social_username
    ...
    def fake_organizations(self, count: int = 10) -> None:
        ...
        self.stdout.write(f"Generating {count} fake organizations...")
    ...
    def real_organizations(self) -> None:
        ...
        for name, domain in orgs:
            ...
            if created:
                self.stdout.write(f'Added "{domain}" organization.')
    ...
    def fake_memberships(self, count: int = 10) -> None:
        self.stdout.write(f"Generating {count} fake memberships...")
    ...
    def fake_current_events(self, count: int = 5, **kwargs: Any) -> None:
        ...
        self.stdout.write(f"Generating {count} fake current events...")
    ...
    def fake_unpublished_events(self, count: int = 5) -> None:
        ...
        them)."""
        self.stdout.write(f"Generating {count} fake unpublished events...")
    ...
    def fake_self_organized_events(self, count: int = 5) -> None:
        ...
        self.stdout.write(f"Generating {count} fake self organized events...")
    ...
    def fake_tasks(self, count: int = 120) -> None:
        self.stdout.write(f"Generating {count} fake tasks...")
    ...
    def fake_unmatched_training_requests(self, count: int = 20) -> None:
        self.stdout.write(f"Generating {count} fake unmatched training requests...")

        for _ in range(count):
            self.fake_training_request(None)

    def fake_duplicated_people(self, count: int = 5) -> None:
        self.stdout.write(f"Generating {count} fake people duplications...")
    ...
    def fake_workshop_requests(self, count: int = 10) -> None:
        self.stdout.write(f"Generating {count} fake workshop requests...")

        curricula = Curriculum.objects.filter(active=True)
        organizations = Organization.objects.all()

        for _ in range(count):
            language = Language.objects.get(subtag="en") if randbool(0.5) else choice(Language.objects.all())
    ...
    def fake_workshop_inquiries(self, count: int = 10) -> None:
        self.stdout.write(f"Generating {count} fake workshop inquiries...")
    ...
    def fake_selforganised_submissions(self, count: int = 10) -> None:
        self.stdout.write(f"Generating {count} fake self-organised submissions...")
    ...
    def fake_consents(self) -> None:
        ...
        self.stdout.write(f"Generating {count} fake consents...")
