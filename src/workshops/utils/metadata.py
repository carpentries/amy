...
from src.workshops.exceptions import WrongWorkshopURL
...
def find_workshop_YAML_metadata(content: str) -> dict[str, str]:
    ...
    for key, value in filtered_metadata.items():
        if isinstance(value, (int, float)):
            ...
            filtered_metadata[key] = f"{value:%Y-%m-%d}"
...
def parse_workshop_metadata(metadata: dict[str, str]) -> WorkshopMetadata:
    ...
    if "lat" in metadata and "lng" in metadata:
        metadata_lat: str | None = metadata.get("lat", "")
        metadata_lng: str | None = metadata.get("lng", "")
    else:
        try:
    ...
    try:
        latitude: float | None = float(metadata_lat.strip())  # type: ignore
    except (ValueError, AttributeError):
        # value error: can't convert string to float
        # attribute error: object doesn't have "split" or "strip" methods
        latitude = None
    try:
        longitude: float | None = float(metadata_lng.strip())  # type: ignore
    except (ValueError, AttributeError):
        # value error: can't convert string to float
        # attribute error: object doesn't have "split" or "strip" methods
        longitude = None
...
def validate_workshop_metadata(metadata: dict[str, str]) -> tuple[list[str], list[str]]:
    ...
    latlng_req = Requirement(
        ...
        "latitude / longitude",
        True,
        rf"^{FRACTION_FMT},\s?{FRACTION_FMT}$",
    )
    ...
    for requirement in requirements:
        ...
        if value_ is None:
            ...
            try:
                if (
                    (required_ or value_)
                    and requirement.match_format is not None
                    and not re.match(requirement.match_format, str(value_))
                ):
                    errors.append(
