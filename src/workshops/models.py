...
class Task(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.event}/{self.person}={self.role}"
    ...
    def clean(self) -> None:
        ...
        if not has_ttt and self.seat_open_training:
            ...
            errors["seat_open_training"] = ValidationError(
                "Cannot mark this person as open applicant, because the TTT event is not marked as open applications.",
                code="invalid",
            )
...
class Award(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.person}/{self.badge}/{self.awarded}/{self.event}"
...
class TrainingRequest(
    ...
    underrepresented = models.CharField(
        ...
        verbose_name="I self-identify as a member of a group that is under-represented in research and/or computing.",
    ...
    def __str__(self) -> str:
        return f"{self.personal} {self.family} <{self.email}> - {self.get_state_display()}"
...
class TrainingProgress(CreatedUpdatedMixin, models.Model):
    ...
    def clean_notes(
        ...
        if (
            ...
            and involvement_type.notes_required
            and not self.trainee_notes
            and not self.notes
        ):
            msg = f'This field is required for activity "{involvement_type}" if there are no notes from the trainee.'
...
class WorkshopRequest(
    ...
    def __str__(self) -> str:
        return (
            f"Workshop request ({str(self.institution or self.institution_other_name)}, "
            f"{self.personal} {self.family}) - {self.get_state_display()}"
        )

    def dates(self) -> str:
        if self.preferred_dates:
            return f"{self.preferred_dates:%Y-%m-%d}"
        else:
