...
class TestMembership(TestBase):
    ...
    def setUpTasks(self) -> None:
        ...
        events = [
            Event(
                slug=f"event-outside-agreement-range-{i}",
                host=self.org_beta,
    ...
    def test_membership_edit_extensions(self) -> None:
        for increment in (10, -5):  # +10 days, -5 days per extension
            with self.subTest(increment=increment):
                ...
                msg = (
                    f"Extension days changed to following: {str_ext} days. New agreement end date: {new_agreement_end}."
                )
...
class TestMembershipCreateRollOver(TestBase):
    ...
    def test_membership_rollovers(self) -> None:
        ...
        with self.assertRaises(Membership.rolled_from_membership.RelatedObjectDoesNotExist):
            self.membership.rolled_from_membership  # noqa: B018
