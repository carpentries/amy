...
from src.extrequests import views
...
urlpatterns = [
    ...
    path(
        "workshop_request/<int:request_id>/",
        include(
            [
                path("", views.WorkshopRequestDetails.as_view(), name="workshoprequest_details"),
                path(
                    "set_state/<slug:state>/", views.WorkshopRequestSetState.as_view(), name="workshoprequest_set_state"
                ),
    ...
    path(
        "workshop_inquiry/<int:inquiry_id>/",
        include(
            [
                path("", views.WorkshopInquiryDetails.as_view(), name="workshopinquiry_details"),
                path(
                    "set_state/<slug:state>/", views.WorkshopInquirySetState.as_view(), name="workshopinquiry_set_state"
                ),
    ...
    path(
        "selforganised_submissions/", views.AllSelfOrganisedSubmissions.as_view(), name="all_selforganisedsubmissions"
    ),
    path(
        "selforganised_submission/<int:submission_id>/",
        include(
            [
                path("", views.SelfOrganisedSubmissionDetails.as_view(), name="selforganisedsubmission_details"),
                path(
                    "set_state/<slug:state>/",
                    views.SelfOrganisedSubmissionSetState.as_view(),
                    name="selforganisedsubmission_set_state",
                ),
                path(
                    "accept_event/",
                    views.SelfOrganisedSubmissionAcceptEvent.as_view(),
                    name="selforganisedsubmission_accept_event",
                ),
