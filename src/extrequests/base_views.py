...
from src.extrequests.utils import get_membership_or_none_from_code
from src.workshops.base_views import AMYCreateView
from src.workshops.models import Curriculum, Tag, WorkshopRequest


class WRFInitial[M2: SelfOrganisedSubmission | WorkshopInquiryRequest | WorkshopRequest]:
    other_object: M2 | None
...
class AMYCreateAndFetchObjectView[
    M: Model,
    M2: Model,
    MF: BaseModelForm,  # type: ignore[type-arg]
](AMYCreateView[MF, M]):
    """AMY-based CreateView extended with fetching a different object based on
    URL parameter."""

    model_other: type[M2] | None
    queryset_other: QuerySet[M2] | None
    other_object: M2 | None
    ...
    def get_other_object(self, queryset: QuerySet[M2] | None = None) -> M2:
    ...
    def get_other_queryset(self) -> QuerySet[M2]:
