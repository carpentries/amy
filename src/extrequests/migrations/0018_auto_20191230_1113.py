...
def mutation(original: str) -> str:
    # It only works because no-one used apostrophe (') in any existing data.
    return original.replace("['", "").replace("']", "").replace("', '", "")


def update_additional_contact(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    ...
    SelfOrganisedSubmission = apps.get_model("extrequests", "SelfOrganisedSubmission")
