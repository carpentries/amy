...
from src.extrequests.forms import WorkshopInquiryRequestBaseForm
from src.extrequests.models import DataVariant, WorkshopInquiryRequest
...
class TestWorkshopInquiryBaseForm(FormTestHelper, TestBase):
    ...
    def _test_dont_know_yet_option[M: Model](
        self, Form: type[ModelForm[M]], field: str, value_normal: int, value_dont_know_yet: int
    ) -> None:
    ...
    def test_dates_validation(self) -> None:
        ...
        data = {
            "preferred_dates": f"{date.today():%Y-%m-%d}",
            "other_preferred_dates": "",
        }
        form = WorkshopInquiryRequestBaseForm(data)
        self.assertNotIn("preferred_dates", form.errors)
        ...
        data = {
            "preferred_dates": f"{date.today():%d-%m-%Y}",
            "other_preferred_dates": "",
        }
        form = WorkshopInquiryRequestBaseForm(data)
        self.assertIn("preferred_dates", form.errors)
