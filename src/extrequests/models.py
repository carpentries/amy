...
from src.workshops.consts import FEE_DETAILS_URL, STR_LONGEST
...
class WorkshopInquiryRequest(
    ...
    def __str__(self) -> str:
        return (
            f"Workshop inquiry ({str(self.institution or self.institution_other_name)}, {self.personal} "
            f"{self.family}) - {self.get_state_display()}"
        )
...
class SelfOrganisedSubmission(
    ...
    def __str__(self) -> str:
        return (
            f"Self-Organised Submission ({str(self.institution or self.institution_other_name)}, {self.personal} "
            f"{self.family}) - {self.get_state_display()}"
        )
