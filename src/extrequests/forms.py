...
class WorkshopRequestBaseForm(forms.ModelForm[WorkshopRequest]):
    ...
    def validate_member_code(self, request: HttpRequest) -> None | dict[str, ValidationError]:
        ...
        # Confirm that membership, partnership, or account benefit is active at the time of submission.
        # Grace period: 60 days before, 0 days after.
        if not any_member_code_valid(member_code, datetime.date.today(), grace_before=60, grace_after=0):
    ...
    def clean(self) -> None:
        ...
        if public_event == "other" and not public_event_other:
            ...
            errors["public_event"] = ValidationError('If you entered data in "Other" field, please select that option.')
        ...
        if (
            (preferred_dates and preferred_dates <= two_months_away) or not preferred_dates
...
class WorkshopInquiryRequestBaseForm(forms.ModelForm[WorkshopInquiryRequest]):
    ...
    def clean(self) -> None:
        ...
        if routine_data and routine_data.filter(unknown=True) and len(routine_data) > 1 or routine_data_other:
            errors["routine_data"] = ValidationError(
                "If you select \"Don't know yet\", you can't select anything else or enter other values."
            )

        if domains and domains.filter(name="Don't know yet") and len(domains) > 1 or domains_other:
...
class TrainingRequestUpdateForm(forms.ModelForm[TrainingRequest]):
    ...
    def validate_member_code(self, request: HttpRequest) -> None | dict[str, ValidationError]:
        ...
        code_is_valid = any_member_code_valid_training(
            member_code,
        ...
        if not code_is_valid and not member_code_override:
            errors["member_code"] = ValidationError(
                "This code is invalid. Tick the checkbox below to ignore this message."
            )
