...
from src.workshops.mixins import AssignmentMixin, CreatedUpdatedMixin, StateMixin
...
class InstructorRecruitment(CreatedUpdatedMixin, AssignmentMixin, models.Model):
    ...
    def __str__(self) -> str:
        return f"Instructor Recruitment Process ({self.get_status_display()}) for event {self.event.slug}"
...
class InstructorRecruitmentSignup(CreatedUpdatedMixin, StateMixin, models.Model):
    ...
    def __str__(self) -> str:
        return f"Instructor Signup applicant {self.person.full_name} for {self.recruitment.event}"
