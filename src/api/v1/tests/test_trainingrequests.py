...
from src.api.v1.views import TrainingRequests
...
from src.workshops.tests.base import consent_to_all_required_consents
...
class TestListingTrainingRequests(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.tr1 = TrainingRequest(
            ...
            reason="I hope to pass on the Gummi Wisdom one day, and to do that I must know how to do it efficiently.",
            user_notes="",
    ...
    def test_selected_ids(self) -> None:
        ...
        self.client.login(username="admin", password="admin")
        response = self.client.get(url, {"ids": f"{self.tr2.pk}"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
