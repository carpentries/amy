...
from src.emails.types import PersonsMergedContext
from src.emails.views import (
...
class TestScheduledEmailReschedule(TestBase):
    @patch("src.emails.forms.datetime", wraps=datetime)
    @override_settings(FLAGS={"EMAIL_MODULE": [("boolean", True)]})
...
class TestScheduledEmailAddAttachment(TestBase):
    @patch("src.emails.controller.s3_client")
    @override_settings(FLAGS={"EMAIL_MODULE": [("boolean", True)]})
