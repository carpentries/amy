...
from src.api.v2.serializers import PersonSerializer
...
class TestMessagesMissingRecipients(TestCase):
    @patch("src.emails.utils.messages.warning")
...
class TestMessagesActionScheduled(TestCase):
    @patch("src.emails.utils.messages.info")
...
class TestMessagesActionUpdated(TestCase):
    ...
    def test_messages_action_updated(self, mock_messages_info: MagicMock) -> None:
        ...
        mock_messages_info.assert_called_once_with(
            request,
            f'Existing <a href="{scheduled_email.get_absolute_url()}">email action ({name})</a> was updated.',
            extra_tags=settings.ONLY_FOR_ADMINS_TAG,
        )
...
class TestMessagesActionCancelled(TestCase):
    ...
    def test_messages_action_cancelled(self, mock_messages_warning: MagicMock) -> None:
        ...
        mock_messages_warning.assert_called_once_with(
            request,
            f'Existing <a href="{scheduled_email.get_absolute_url()}">email action ({name})</a> was cancelled.',
            extra_tags=settings.ONLY_FOR_ADMINS_TAG,
        )
...
class TestMapSingleApiUriToSerializedModelOrValue(TestCase):
    @patch("src.emails.utils.scalar_value_from_type")
...
class TestMapApiUriToSerializedModelOrValue(TestCase):
    @patch("src.emails.utils.map_single_api_uri_to_serialized_model_or_value")
...
class TestBuildContextFromDict(TestCase):
    @patch("src.emails.utils.map_api_uri_to_serialized_model_or_value")
...
class TestBuildContextFromList(TestCase):
    @patch("src.emails.utils.map_single_api_uri_to_serialized_model")
    @patch("src.emails.utils.map_single_api_uri_to_value")
