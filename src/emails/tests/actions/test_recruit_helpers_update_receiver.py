...
from src.emails.signals import RECRUIT_HELPERS_SIGNAL_NAME, recruit_helpers_update_signal
...
class TestRecruitHelpersUpdateReceiver(TestCase):
    ...
    def test_multiple_previously_scheduled_emails(
        ...
        mock_email_controller.update_scheduled_email.assert_not_called()
        mock_logger.warning.assert_called_once_with(
            f"Too many scheduled emails for signal {signal} and generic_relation_obj={event!r}. Can't update them."
        )
