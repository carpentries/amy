...
from src.emails.signals import INSTRUCTOR_TASK_CREATED_FOR_WORKSHOP_SIGNAL_NAME
from src.emails.types import StrategyEnum
from src.workshops.models import Event, Organization, Person, Role, Tag, Task


class TestInstructorTaskCreatedForWorkshopStrategy(TestCase):
    def setUp(self) -> None:
    ...
    def test_strategy_create(self) -> None:
        ...
        # Act
        result = instructor_task_created_for_workshop_strategy(self.task)
...
class TestRunInstructorTaskCreatedForWorkshopStrategy(TestCase):
    ...
    @patch("src.emails.actions.instructor_task_created_for_workshop.instructor_task_created_for_workshop_signal")
    def test_strategy_calls_create_signal(
        self,
        mock_instructor_task_created_for_workshop_signal: MagicMock,
    ) -> None:
        # Arrange
        ...
        # Assert
        mock_instructor_task_created_for_workshop_signal.send.assert_called_once_with(
            sender=self.task,
            request=request,
            task=self.task,
            person_id=self.task.person.pk,
    ...
    @patch("src.emails.actions.instructor_task_created_for_workshop.instructor_task_created_for_workshop_update_signal")
    ...
    @patch("src.emails.actions.instructor_task_created_for_workshop.instructor_task_created_for_workshop_cancel_signal")
    ...
    def test_invalid_strategy_no_signal_called(
        self,
        mock_instructor_task_created_for_workshop_cancel_signal: MagicMock,
        mock_instructor_task_created_for_workshop_update_signal: MagicMock,
        mock_instructor_task_created_for_workshop_signal: MagicMock,
