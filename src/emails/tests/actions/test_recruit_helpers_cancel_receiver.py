...
from src.emails.signals import RECRUIT_HELPERS_SIGNAL_NAME, recruit_helpers_cancel_signal
...
class TestRecruitHelpersCancelReceiver(TestCase):
    ...
    def test_disabled_when_no_feature_flag(self, mock_logger: MagicMock) -> None:
        ...
        with self.settings(FLAGS={"EMAIL_MODULE": [("boolean", False)]}):
            # Act
            recruit_helpers_cancel_receiver(None, request=request)
            # Assert
            mock_logger.debug.assert_called_once_with(
                "EMAIL_MODULE feature flag not set, skipping recruit_helpers_cancel"
            )

    def test_receiver_connected_to_signal(self) -> None:
        # Arrange
        original_receivers = recruit_helpers_cancel_signal.receivers[:]

        # Act
        # attempt to connect the receiver
        recruit_helpers_cancel_signal.connect(recruit_helpers_cancel_receiver)
        new_receivers = recruit_helpers_cancel_signal.receivers[:]
