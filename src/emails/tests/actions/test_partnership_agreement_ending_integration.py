...
class TestPartnershipAgreementEndingReceiverIntegration(TestBase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def test_integration_create_via_account_owners_update(self) -> None:
        ...
        template = EmailTemplate.objects.create(
            ...
            signal=PARTNERSHIP_AGREEMENT_ENDING_SIGNAL_NAME,
...
class TestPartnershipAgreementEndingUpdateReceiverIntegration(TestBase):
    def setUp(self) -> None:
        super().setUp()
        self._setUpUsersAndLogin()
    ...
    @override_settings(FLAGS={"EMAIL_MODULE": [("boolean", True)], "SERVICE_OFFERING": [("boolean", True)]})
    def test_integration_update_via_partnership_extend(self) -> None:
        # Arrange
        signal = PARTNERSHIP_AGREEMENT_ENDING_SIGNAL_NAME
        template = EmailTemplate.objects.create(
            name="Test Email Template",
        ...
        scheduled_email = ScheduledEmail.objects.get(template=template)
        original_scheduled_at = scheduled_email.scheduled_at

        url = reverse("partnership-extend", args=[self.partnership.pk])
        new_agreement_end = self.partnership.agreement_end + timedelta(days=180)
        data = {
            "agreement_start": self.partnership.agreement_start.isoformat(),
            "agreement_end": self.partnership.agreement_end.isoformat(),
            "new_agreement_end": new_agreement_end.isoformat(),
            "extension": 180,
            "comment": "",
        }
        ...
        self.assertEqual(self.partnership.agreement_end, new_agreement_end)
        scheduled_email.refresh_from_db()
        self.assertEqual(scheduled_email.state, ScheduledEmailStatus.SCHEDULED)
        self.assertNotEqual(scheduled_email.scheduled_at, original_scheduled_at)
        latest_log = ScheduledEmailLog.objects.filter(scheduled_email=scheduled_email).order_by("-created_at").first()
        assert latest_log
        self.assertEqual(latest_log.details, f"Updated {signal}")


class TestPartnershipAgreementEndingCancelIntegration(TestBase):
    def setUp(self) -> None:
        super().setUp()
        self._setUpUsersAndLogin()
