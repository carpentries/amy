...
from src.emails.actions.base_action import (
    BaseAction,
    BaseActionCancel,
    BaseActionUpdate,
    feature_flag_enabled,
)
from src.emails.controller import (
...
class TestFeatureFlagEnabled(TestCase):
    ...
    def test_feature_flag_enabled__missing_request(self, mock_logger: MagicMock) -> None:
        ...
        mock_logger.debug.assert_called_once_with(
            f"Cannot check {self.feature_flag} feature flag, `request` parameter to {self.signal_name} is missing"
        )
...
class BaseActionForTesting(BaseAction):
    ...
    def get_scheduled_at(self, **kwargs: Any) -> datetime:
        return datetime(2023, 10, 18, 23, 00, tzinfo=UTC)
...
class TestBaseAction(TestCase):
    @patch("src.emails.actions.base_action.feature_flag_enabled", return_value=True)
    @patch("src.emails.actions.base_action.person_from_request")
    @patch("src.emails.actions.base_action.EmailController.schedule_email")
    ...
    @patch("src.emails.actions.base_action.feature_flag_enabled", return_value=False)
    @patch("src.emails.actions.base_action.EmailController.schedule_email")
...
class TestBaseActionUpdate(TestCase):
    ...
    @patch("src.emails.actions.base_action.feature_flag_enabled", return_value=True)
    @patch("src.emails.actions.base_action.person_from_request")
    @patch("src.emails.actions.base_action.EmailController.update_scheduled_email")
    ...
    def test_call__no_scheduled_email(
        ...
        mock_logger.warning.assert_called_once_with(
            f"Scheduled email for signal {instance.signal} and generic_relation_obj={event!r} does not exist."
        )
    ...
    @patch("src.emails.actions.base_action.feature_flag_enabled", return_value=True)
    @patch("src.emails.actions.base_action.EmailController.update_scheduled_email")
    @patch("src.emails.actions.base_action.messages_missing_template_link")
...
class TestBaseActionCancel(TestCase):
    ...
    @patch("src.emails.actions.base_action.feature_flag_enabled", return_value=True)
    @patch("src.emails.actions.base_action.person_from_request")
    @patch("src.emails.actions.base_action.EmailController.cancel_email")
