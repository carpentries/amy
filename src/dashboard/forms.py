...
class GetInvolvedForm(forms.ModelForm[TrainingProgress]):
    ...
    def user_may_create_submission(self, user: Person) -> bool:
        ...
        if num_existing == 0:
            ...
            return num_asked_to_repeat == num_existing
    ...
    def clean_url(self) -> str:
        ...
        if involvement_type and involvement_type.name == "GitHub Contribution":
            if not url:
                ...
                msg = f'This field is required for activity "{involvement_type.display_name}".'
...
class SignupForRecruitmentForm(forms.ModelForm[InstructorRecruitmentSignup]):
    user_notes = forms.CharField(
        ...
        "Please include any accessibility support that the host organisation should provide "
    ...
    def clean(self) -> None:
        ...
        except CommunityRole.DoesNotExist as e:
            raise ValidationError("You don't have an active Instructor Community Role") from e
        ...
        if (
            ...
            raise ValidationError(
                f"Selected event dates conflict with events: {', '.join(task.event.slug for task in conflicting_tasks)}"
            )
