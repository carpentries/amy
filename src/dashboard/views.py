...
from src.consents.forms import TermBySlugsForm
from src.consents.models import Consent, Term, TermEnum
from src.dashboard.filters import UpcomingTeachingOpportunitiesFilter
...
class SignupForRecruitment(
    ...
    def form_valid(self, form: SignupForRecruitmentForm) -> HttpResponse:
        ...
        if conflicting_signups := InstructorRecruitmentSignup.objects.exclude(recruitment=recruitment).filter(
            ...
            messages.warning(
                self.request,
                f"You have applied to other workshops on the same dates: {', '.join(gen)}",
            )
        ...
        if form.is_valid():
            ...
            if len(results_combined) == 1 and not form.cleaned_data["no_redirect"]:
                ...
                if isinstance(result, Comment):
                    ...
                    return redirect(
                        result.content_object.get_absolute_url() + f"#c{result.id}"  # type: ignore[union-attr]
                    )
