...
def search_closed_prs_in_milestone(
    ...
    *,
    ...
    json = response.json()
    results = cast(list[dict[str, Any]], json["items"])
...
def pretty_milestone(milestone: dict[str, Any]) -> str:
...
def pretty_pull_request(pr: dict[str, Any]) -> str:
    return f"* {pr['title']} - [#{pr['number']}]({pr['html_url']}) by @{pr['user']['login']}"


def find_milestone_number_in_pr(pr: dict[str, Any]) -> int:
    return cast(int, pr["milestone"]["number"])


def labels_contain(label_name: str, labels: list[dict[str, Any]]) -> bool:
    return label_name in {label["name"] for label in labels}


def find_bugs_features_in_prs(prs: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def pretty_bugfixes(bugfixes: list[dict[str, Any]]) -> str:
    return "### Bugfixes\n" + "\n".join(pretty_pull_request(bugfix) for bugfix in bugfixes) + "\n"


def pretty_features(features: list[dict[str, Any]]) -> str:
